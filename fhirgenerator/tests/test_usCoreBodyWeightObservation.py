'''Testing US Core Body Weight Observation generator'''
...
def testUSCoreBodyWeightObservationGenerator():
    '''Test function for US Core Body Weight Observation Generator'''
    ...
    for detail in config_dict['resourceDetails']:
        if detail['fhirResource'] == 'USCoreBodyWeightObservation':
            observation_resource_details.append(detail)

    for i, detail in enumerate(observation_resource_details):
        created_resource = generateUSCoreBodyWeightObservation(detail, patient_id, config_dict['startDate'], config_dict['days'])

        assert created_resource['resourceType'] == 'Observation'
        assert created_resource['meta']['profile'] == ['http://hl7.org/fhir/us/core/StructureDefinition/us-core-body-weight']
        ...
        assert created_resource['code']['coding'][0] == {'system': 'http://loinc.org', 'code': '29463-7'}
        ...
        with open(f'fhirgenerator/tests/output/test_usCoreBodyWeightObservation_{i}.json', 'wb') as outfile:
