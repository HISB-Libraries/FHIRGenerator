'''Testing US Core Body Temperature Observation generator'''
...
def testUSCoreBodyTemperatureObservationGenerator():
    '''Test function for US Core Body Temperature Observation Generator'''
    ...
    for detail in config_dict['resourceDetails']:
        if detail['fhirResource'] == 'USCoreBodyTemperatureObservation':
            observation_resource_details.append(detail)

    for i, detail in enumerate(observation_resource_details):
        created_resource = generateUSCoreBodyTemperatureObservation(detail, patient_id, config_dict['startDate'], config_dict['days'])

        assert created_resource['resourceType'] == 'Observation'
        assert created_resource['meta']['profile'] == ['http://hl7.org/fhir/us/core/StructureDefinition/us-core-body-temperature']
        ...
        assert created_resource['code']['coding'][0] == {'system': 'http://loinc.org', 'code': '8310-5'}
        ...
        with open(f'fhirgenerator/tests/output/test_usCoreBodyTemperatureObservation_{i}.json', 'wb') as outfile:
